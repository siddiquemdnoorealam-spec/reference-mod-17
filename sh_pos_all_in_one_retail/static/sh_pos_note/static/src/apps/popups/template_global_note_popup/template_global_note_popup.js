...
export class TemplateGlobalNotePopupWidget extends AbstractAwaitablePopup {
    static template = "sh_pos_note.TemplateGlobalNotePopupWidget";
    ...
    async confirm() {
        ...
        self.pos.get_order().set_global_note(value);
        if (self.pos.config.hide_extra_note_checkbox && this.value.store_checkbox) {
            var value = this.value.notes;
            var added_note = this.value.notes.split(",");
