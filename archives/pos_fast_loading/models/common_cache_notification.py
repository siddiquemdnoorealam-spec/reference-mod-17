...
try:
    # Import necessary modules from PyMongo library
    from pymongo import MongoClient
    from pymongo.errors import ServerSelectionTimeoutError

except Exception as e:
    # If PyMongo is not installed, log an error with a descriptive message
...
PRODUCTFIELD = ['display_name', 'list_price', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_ids', 'taxes_id', 'barcode', 'default_code', 'to_weight', 'uom_id', 'description_sale', 'description', 'product_tmpl_id', 'tracking', 'active', 'available_in_pos','optional_product_ids', 'attribute_line_ids','combo_ids']
    ...
    def get_common_changes(self):
        ...
        if mongo_server_rec:
            ...
            if mongo_server_rec.partner_field_ids:
                partner_fields = partner_fields + [str(data.name) for data in mongo_server_rec.partner_field_ids]
            ...
            if mongo_server_rec.product_all_fields:
                if mongo_server_rec.load_pos_data_from == 'postgres':
                    fields = self.env['ir.model'].sudo().search([('model', '=', 'product.product')]).field_id
