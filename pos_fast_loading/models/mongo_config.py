...
class MongoServerConfig(models.Model):
    ...
    def load_rem_product(self, page):
        mongo_server_rec = self.search([('active_record', '=', True)], limit=1)
        _logger.info(
            f"####### loading remaining product  page - {page} ############")
    ...
    def load_rem_customer(self, page):
        mongo_server_rec = self.search([('active_record', '=', True)], limit=1)
        _logger.info(
            f"####### loading remaining customers  page - {page} ############")
        ...
        if mongo_server_rec:
            ...
            try:
                ...
                if new_cache_records:
                    ...
                    self.env['pos.session']._process_pos_ui_product_product(
                        product_res)
                    data_dict.update({
    ...
    def return_product_fields(self, mongo_server_rec):
        fields = ['active', 'display_name', 'list_price', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_ids', 'taxes_id',
                  'barcode', 'default_code', 'to_weight', 'uom_id', 'description_sale', 'description',
                  'product_tmpl_id', 'tracking', 'available_in_pos', 'optional_product_ids', 'image_128', 'attribute_line_ids', 'combo_ids']
        if mongo_server_rec.product_field_ids:
    ...
    def load_product_data_postgres(self, p_data, product_fields):
        ...
        self.env['pos.session']._process_pos_ui_product_product(pro_data)
